...
int SvServerClient::connectToHost()
{
  try
  {
    if (this->outIp == "") throw CE_WRONG_IP;
    if (this->outPort == 0) throw(CE_WRONG_PORT);
    ...
    if (!this->connected)
      {    
        ...
        else /*return CE_BAD_CONNECTION;*/ throw(CE_BAD_CONNECTION);
      }
  }
      ...
      case false:
        if (this->clientSocket->bytesAvailable() > 0)
        {
          ...
          if(this->showSymbols)
          {
            ...
            {
              if((arrBlock.at(i) > 31) | (arrBlock.at(i) == 10) |(arrBlock.at(i) == 13)) str = str + arrBlock.at(i);
              else 
              {
                s.setNum(arrBlock.at(i), 16);
                if(s.length() == 1) s = '0' + s;
                str = str + "0x" + s /*+ arrBlock.at(i*2 + 1)*/ + " "; 
              }
            }
          }
          else
          {
            ...
            for (int i=0; i < arrBlock.size(); i=i+2)
            {
...
//          if((arrBlock.at(i) > 31) | (arrBlock.at(i) == 10) |(arrBlock.at(i) == 13)) str = str + arrBlock.at(i);
//          else str = str + '[' + arrBlock.at(i*2) + arrBlock.at(i*2 + 1) + ']'; 
          QString s = "";
          int u = 0;
          char o = arrBlock.at(i);
          memcpy(&u, &o, 1);
